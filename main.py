...
def create_app(master):
    def show_answer():
        ...
        answer_label.insert("end","-"*168 + "\n\n" + "user: " + message)
        clear_message()
        answer_question(message)

    message_label = tk.Label(master, text="This Program Has Written by Younes Veisi:")
    message_label.pack()

    message_input = create_multi_line_input_with_placeholder(
        master, "Type your message here")
    message_input.pack()
